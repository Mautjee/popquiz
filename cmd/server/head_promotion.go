...
func promoteDisconnectedHeads(database *sql.DB, broker *sse.Broker) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&h.ID, &h.TeamID, &h.GameID); err != nil {
			continue
		}
	...
	for _, h := range heads {
		...
		log.Printf("Promoted player %d to head of team %d", newHeadID, h.TeamID)
		...
		eventData := fmt.Sprintf(`{"team_id":%d,"new_head_player_id":%d}`, h.TeamID, newHeadID)
		broker.Publish(roomCode, sse.Event{Type: "head_change", Data: eventData})
		broker.Publish("admin:"+roomCode, sse.Event{
			Type: "head_changed",
			Data: fmt.Sprintf(`{"team_id":%d,"new_head_player_id":%d}`, h.TeamID, newHeadID),
		})
	}
}